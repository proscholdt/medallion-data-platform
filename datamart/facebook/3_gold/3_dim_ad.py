...
DEST_FOLDER = "source_facebook/dim_ad"
OUTPUT_FILE_NAME = "dim_ad.parquet"
...
# Listar arquivos EXATAMENTE da pasta facebook_ad
blobs = container_client.list_blobs()
arquivos_parquet = [
    b.name for b in blobs
    if b.name.startswith(SOURCE_FOLDER + "/") and b.name.endswith(".parquet")
]
...
for blob_name in arquivos_parquet:
    try:
        ...
        df = pl.read_parquet(BytesIO(blob_data)).select(["ad_id", "ad_name"])
        tabelas.append(df)
    except Exception as e:
        print(f"⚠️ Erro ao processar {blob_name}: {e}")

# Unir, deduplicar e salvar
if tabelas:
